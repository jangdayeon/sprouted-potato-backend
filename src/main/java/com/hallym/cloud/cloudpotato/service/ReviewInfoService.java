...
public class ReviewInfoService {
    ...
    @Transactional(readOnly = true)
    public List<String> findByResultAI(){
        //리뷰작성한 책 모두 가져오기
        List<String> list1 = reviewInfoRepository.findDistinctIsbn();
        List<String[]> list2 = new ArrayList<>(list1.size());
        for(int i=0;i<list1.size();i++){
            double positive =  reviewInfoRepository.countByIsbnAndResultAI(list1.get(i), "긍정");
            double allReviewCount = reviewInfoRepository.countByIsbn(list1.get(i));
            double positiveStats = (positive / allReviewCount) * 100;

            list2.add(new String[]{list1.get(i), String.valueOf(Math.round(positiveStats))});
        }
